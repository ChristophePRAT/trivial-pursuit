...
export default function Page() {
    ...
    const alreadySelectedCountries = players.map((player) => {
        return player.name;
    });
    ...
    return (
        <div className="flex flex-col justify-center items-center w-screen text-center">
            <h1 className="text-5xl font-bold">Choisissez votre pays</h1>
            <div className="flex flex-col rounded-xl border-2 border-slate-200 p-4 m-3">
                    ...
                    return (
                        <div key={index} className="flex flex-row items-center">
                            ...
                            <input
                                ...
                                name="remove"
                                value="Enlever"
                                className="m-2 cursor-pointer text-red-500"
                                onClick={() => {
                                    const newPlayers = gameContext.players;
                ...
                <div className="flex flex-row items-center">
                    <label
                        htmlFor="name"
                        className="m-2"
                    >{`Joueur ${players.length + 1}:`}</label>
                    <Datalist
                        onChange={(e: any) => {
                            setNewPlayerName(e.target.value);
                        }}
                        alreadySelectedCountries={alreadySelectedCountries}
                        selected={newPlayerName}
                    />
                    <input
                        type="submit"
                        name="submit"
                        value="Ajouter"
                        className="rounded-lg p-2 m-2 border-red-500 border-2 cursor-pointer"
                        onClick={addPlayer}
                    />
                </div>
                {players.length < 2 ? (
                    <p className="text-red-500 m-2">
                        Vous devez avoir au moins deux camarades pour commencer
                        la partie
                    </p>
                ) : (
                    <Link
                        href="/game"
                        className="bg-red-500 rounded-lg text-white p-4 font-medium m-2"
                    >
                        Commencer la partie
                    </Link>
                )}
